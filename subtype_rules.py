...
if __name__ == '__main__':
    ...
    subtype = list(pd.read_excel('results_final/cel7_subtypes.xlsx')['ncbi_pred_class'])
...
class Cel7MSA():
    ...
    def get_freq(self, analysis='amino', include_gaps=True):
        ...
        self.cbh_freq = self._get_aa_freq(self.cbh_msa, analysis='amino', include_gaps=include_gaps)
        self.egl_freq = self._get_aa_freq(self.egl_msa, analysis='amino', include_gaps=include_gaps)
    ...
    def get_consensus_sequences(self):
        ...
        # Loop through each position and determine the 
    ...
    def get_rules(self, analysis='amino'):
        ...
        for i in range(len(cbh_freq.columns)):
            ...
            if cbh_cons_type != '-':
                ...
                rule.append('{0}=>CBH, not {0}=>EGL'.format(key))
            ...
            if egl_cons_type != '-':
                ...
                rule.append('not {0}=>CBH, {0}=>EGL'.format(key))
            ...
            if cbh_cons_type != egl_cons_type and '-' not in [cbh_cons_type, egl_cons_type]:
                ...
                rule.append('{0}=>CBH, {1}=>EGL'.format(keyX, keyZ))
    ...
    def site_plot(self, site, savefig=False):
        ...
        plt.title('POS{0}'.format(site), **title_font)
        ...
        if savefig:
           plt.savefig('pos{0}.pdf'.format(site))
        plt.show()
...
def distance(x,y):
        '''Returns the euclidean distance between 2 3D vectors.'''
...
# Calculate distances 
# between closest atoms of protein residues and glycosyl residues
store = []
...
dist_store.to_excel('results_final/residue_distances.xlsx')
...
if __name__ == '__main__':
    ...
    # Save rules
    rules_amino.to_excel('results_final/rules/rules_amino.xlsx')
    rules_type.to_excel('results_final/rules/rules_type.xlsx')
    rules.to_excel('results_final/rules/rules_all.xlsx')
    ...
    bioinf.combine_fasta(['CBH consensus', 'EG consensus'],
                         [consensus_cbh, consensus_egl],
